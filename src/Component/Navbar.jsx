...
const Navbar = ({onSearch}) => {
 ...
 const getStepClasses = (stepId) => {
  ...
  const showLogister = location.pathname === '/' || location.pathname === '/home';
      ...
      <nav className='flex flex-col md:flex-row items-center gap-1 md:gap-10  bg-black text-white h-auto md:h-28 fixed w-full top-0 left-0 z-50 px-4 py-3'>
          ...
          {showSearchbar && (
            <input 
                ...
                className='mt-3 md:mt-0 h-10 w-24 focus:w-full md:w-80 rounded-full pl-4 text-gray-300 font-opensans bg-zinc-900'
            />
          )}
