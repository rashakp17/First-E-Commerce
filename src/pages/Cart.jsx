      ...
      <div className='flex flex-col md:flex-row  justify-between '>
        ...
        <div>
          ...
          {cartItems.map((item)=>(
            ...
            <div key={item.id} className='flex items-center mx-5 flex-row h-36 text-sm md:w-[500px] justify-center my-4 md:ml-[300px]'>
              <img src={item.image} alt={item.title} className='h-20 w-20  mr-5'/>
              <div className='flex-1 '>
                <h2 className='text-white text-sm my-5 font-opensans md:text-sm font-semibold'>{item.title}</h2>
              ...
              <p className='text-white font-normal font-poppins text-start text-sm md:text-sm '>₹{item.price}</p>
            </div>
          ))}
        </div>
          {cartItems.length > 0 && (
            <div className=' flex flex-col gap-2  '>
             <OrderSummary/>
              <button className='md:w-48 w-80 text-white bg-gray-500 text-base h-10 rounded-lg hover:bg-gray-700 mb-2 active:bg-gray-400 active:bg-opacity-50' ><Link to={`/checkout`}>CONTINUE</Link> </button>
              <button
              className='md:w-48 w-80 text-white bg-gray-500 text-base h-10 rounded-lg hover:bg-gray-700 mb-4 active:bg-gray-400 active:bg-opacity-50'
